...
import { GenerationSummaryCards } from './components/GenerationSummaryCards';
...
import { analyzeUserStory, generateTestCases } from './services/api';
import {
  Sparkles,
  Loader2,
  AlertCircle,
...
export function App() {
  ...
  const handleAnalyze = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Error al conectar con Jira.');
      setUiState('IDLE');
    }
  };
        ...
        <main className="flex-1 p-6 max-w-7xl w-full mx-auto space-y-6">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400">• Generación Determinista & Trazable</span>
                </div>
                <h1 className="text-2xl font-extrabold text-white tracking-tight">
                  Test Case Generator Agent
                </h1>
                <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                  Consulta automáticamente la Historia de Usuario y sus Criterios de Aceptación desde Jira, valida la suficiencia de información y sintetiza casos de prueba estructurados.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                {uiState === 'MANUAL_MODE' ? (
                  <button
                    ...
                    className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-cyan-300 rounded-xl text-xs font-semibold border border-slate-700 transition-all flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl text-xs font-semibold border border-slate-700 transition-all flex items-center gap-1.5"
                  >
          ...
          {error && (
            <div className="p-4 rounded-2xl bg-rose-950/40 border border-rose-800/60 text-rose-300 text-xs flex items-start gap-3 shadow-lg animate-fadeIn">
              ...
              <div>
                <span className="font-semibold block text-rose-200">Error en la integración / generación</span>
