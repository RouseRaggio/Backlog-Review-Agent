import { ReviewRequest, ReviewResponse } from '../types/backlog';

const API_BASE_URL = import.meta.env.VITE_BACKLOG_API_URL || '';

export async function checkBacklogHealth(): Promise<boolean> {
  try {
...
export async function executeReview(payload: ReviewRequest): Promise<ReviewResponse> {
  ...
  if (!response.ok) {
    let errorDetail = 'Error al ejecutar la auditoría de backlog';
    try {
